...
def head_file(size: int, relative_path: str):
    abs_path = f"/home/user/module_2/{relative_path}"
    with open(abs_path, 'r') as file:
        file_content = file.read()
        result_size = len(file_content[:size])
        result_text = escape(file_content[:size])

    response = f"{abs_path} {result_size}<br>{result_text}"
    return response
